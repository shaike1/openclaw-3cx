...
async function fetchLatestRelease() {
  try {
    const response = await fetch(
      'https://api.github.com/repos/shaike1/openclaw-3cx/releases/latest',
      {
        headers: {
...
function showManualInstructions(release) {
  ...
  console.log(chalk.bold('   curl -sSL https://raw.githubusercontent.com/shaike1/openclaw-3cx/main/install.sh | bash\n'));
...
export async function updateCommand() {
  ...
  if (isGitRepo(projectRoot)) {
    ...
    try {
      ...
      console.log(chalk.gray('Visit https://github.com/shaike1/openclaw-3cx for manual update\n'));
    }
  }
}
